...
<div class="container col-lg-8">
  <div class="card ukf-card mb-4">
    <div class="ukf-head">
      <span class="ukf-icon"><i class="fas fa-layer-group"></i></span>
      <div>
        <h5 class="ukf-title">Data Unit Kompetensi</h5>
        <div class="ukf-sub">Isi unit dan jam pelajaran untuk satu jenis kursus</div>
      </div>
    </div>

    <div class="card-body pt-3">
      <form action="<?= base_url('uk/tambah') ?>" method="POST" id="formUk">
        ...
        <div class="form-group mb-3">
          ...
          <select class="form-control ukf-input" id="rombel" name="rombel" required>
            ...
            foreach ($rombelList as $row) { ?>
              <option value="<?= $row->Id ?>"><?= $row->Namarombel . ' - ' . $row->Kelas ?></option>
            <?php } ?>
          </select>
        </div>
        ...
        <div class="form-group mb-1">
          ...
          <input type="text" inputmode="numeric" class="form-control ukf-input ukf-total" id="jptotal"
                 name="jptotal" placeholder="0" readonly required>
